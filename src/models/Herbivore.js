...
export default class Herbivore extends Animal {
  ...
  update(ecosystem) {
    ...
    if (threats.length > 0) {
      // Вычисляем центр масс всех угроз
      let avgX = 0, avgY = 0
      ...
      // Устанавливаем цель — центр масс угроз
      this.target = { x: avgX, y: avgY }
      this.targetType = 'threat'
    } else if (this.targetType === 'threat') {
      // Если ранее была угроза, но теперь нет — сбрасываем цель
      this.resetTarget()
    }
    ...
    switch (this.targetType) {
      case 'threat': {
        ...
        if (len > this.perception * 1.2) {
          // Убежал достаточно далеко — сбрасываем цель
          this.resetTarget()
        } else if (len > 1) {
  ...
  if (len < this.radius + plant.radius) {
    if (plant.tryRecharge()) {
      // растение ушло в перезарядку, травоядное получает энергию и сбрасывает цель
      this.eat(20)
      this.resetTarget()
    } else {
      // растение не ушло в перезарядку, считаем, что оно съедено и недоступно
